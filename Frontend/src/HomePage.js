...
const HomePage = () => {
  ...
  const filteredPosts = posts.filter(post => {
    const matchesSearch = !displayedSearchQuery || (
      post.title?.toLowerCase().includes(displayedSearchQuery.toLowerCase()) ||
      post.description?.toLowerCase().includes(displayedSearchQuery.toLowerCase()) ||
      post.make?.toLowerCase().includes(displayedSearchQuery.toLowerCase()) ||
      post.model?.toLowerCase().includes(displayedSearchQuery.toLowerCase())
    );
    ...
    const matchesFilters = (
      ...
      (!filters.minSeats || post.seatingCapacity >= parseInt(filters.minSeats)) &&
      (!filters.maxSeats || post.seatingCapacity <= parseInt(filters.maxSeats)) &&
      (!filters.make || post.make.toLowerCase().includes(filters.make.toLowerCase())) &&
      (!filters.condition || post.condition.toLowerCase() === filters.condition.toLowerCase())
    );
  ...
  const Carousel = ({ images }) => {
    ...
    const handlePrev = () => {
      console.log("Carousel previous button clicked, current index:", currentIndex);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="flex flex-col items-center pt-10 px-4">
        <h1 className="text-5xl font-bold mb-8 bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-cyan-600">
          Vehicle Marketplace
        </h1>
        ...
        <div className="w-full max-w-4xl mb-8">
          ...
          `}>
            <div className="pl-6">
              <Search size={24} className={`transition-colors duration-300 ${isFocused ? 'text-blue-600' : 'text-gray-400'}`} />
            </div>
            <input
              type="text"
            ...
            <button
              ...
              className="px-6 py-4 text-white bg-blue-600 hover:bg-blue-700 transition-colors rounded-r-2xl"
            >
              Search
            </button>
            ...
            <button
              onClick={() => {
                console.log("Filters button clicked, showFilters:", !showFilters);
                setShowFilters(!showFilters);
              }}
              className="px-6 py-4 text-blue-600 hover:text-blue-800 font-medium"
            >
              Filters
            </button>
          </div>
